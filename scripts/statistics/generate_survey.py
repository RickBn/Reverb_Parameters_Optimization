...
for c_col, complexity in enumerate(randomized_dict.keys()):
    ...
    for r_row, room in enumerate(randomized_dict[complexity].keys()):
        r_idx = r_row * (int(complexity) + 2)
        worksheet.merge_range(r_idx + 1,
                              c_idx,
                              r_idx + 1,
                              c_idx + 1,
                              room,
                              merge_format)
        worksheet.write(r_idx + 2, c_idx, "same_room", cell_format)
        worksheet.write_blank(r_idx + 2, c_idx + 1, None, cell_format)
